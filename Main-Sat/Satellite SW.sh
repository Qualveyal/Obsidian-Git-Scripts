...
# === CONFIGURATION ===
# Change this for each device: "pc1-satellite" or "pc2-satellite"
MY_BRANCH="sat-1" 
VAULT_PATH="/home/qual/Code/Vault-Test-Git"
RECOVERY_LOG="${MY_BRANCH}_Recovery_Log.md" # Unique log for this device
...
echo -e "${BLUE}=== Starting Daily Sync ($MY_BRANCH) ===${NC}"
cd "$VAULT_PATH" || { echo -e "${RED}Vault path not found!${NC}"; exit 1; }

# 0. Ensure we are on our satellite branch
CURRENT_BRANCH=$(git rev-parse --abbrev-ref HEAD)
if [ "$CURRENT_BRANCH" != "$MY_BRANCH" ]; then
    echo -e "${YELLOW}Switching to branch $MY_BRANCH...${NC}"
    git checkout "$MY_BRANCH" || git checkout -b "$MY_BRANCH"
fi
...
if [[ -n $(git status -s) ]]; then
    ...
    git add .
    git commit -m "$MY_BRANCH: Manual Sync $(date "+%Y-%m-%d %H:%M")"
fi

# 2. Push to Satellite (Safe Backup)
echo -e "${BLUE}Pushing local state to $MY_BRANCH...${NC}"
git push origin "$MY_BRANCH"

# 3. Fetch Main (The Authority)
...
if ! git merge --no-commit --no-ff origin/main > /dev/null 2>&1; then
    CONFLICT_DETECTED=1
    echo -e "${RED}⚠️  CONFLICT DETECTED - Main has authoritative updates!${NC}"
...
# 5. Execute Merge (Authority Strategy)
